...
- **GUI 버전 개선 사항**
   ...
   - **리빌드 시스템 전면 개편 (Rebuild Manager 도입):**
       ...
       -   **자동 매칭:** 특정 폴더를 지정하면, 교체할 오디오 파일들을 내부 사운드 이름과 비교하여 자동으로 찾아 매칭합니다.
...
**2. GUI 조작**:
   ...
   - **오디오 리빌드 (교체)**:
      ...
      - **파일 교체 및 옵션 지정**:
         ...
         - **자동 매칭:** `Batch Tools`의 **`Auto-Match from Folder`** 버튼을 눌러 특정 폴더 내에서 이름이 일치하는 파일들을 자동으로 찾아 한 번에 교체합니다.
