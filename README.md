...
- **GUI 버전 개선 사항**
   ...
   - **오디오 미리듣기 시스템:** 
       ...
       -   **자동 재생 안정성 강화:** 빠른 목록 스크롤 시 발생하던 재생 끊김 및 엔진 오류를 방지하기 위해 입력 지연(Debounce) 처리와 리소스 정리 대기 로직을 적용했습니다.
   - **Strings Bank 통합 지원:** 
   ...
   - **성능 최적화 및 안정성 강화:** 
       -   **파일 액세스 구조 개선 (Caching):** 파일 분석 시 열어둔 핸들을 메모리에 유지하여, 재생이나 추출 시 파일을 다시 읽는 과정을 생략함으로써 대용량 파일에서의 반응 속도를 극대화했습니다.
       -   **일괄 추출 방식 개선:** 동일한 컨테이너 내의 파일들을 한 번에 묶어서 처리하는 로직을 적용하여, 컨테이너를 반복적으로 열고 닫는 오버헤드를 제거하고 대량 추출 속도를 비약적으로 높였습니다.
...
이번 업데이트는 **체감 성능**과 **반응 속도**를 개선하는 데 집중했습니다. 내부 엔진에 **캐싱 시스템**을 도입하여 대용량 파일을 다룰 때 발생하던 딜레이를 제거했습니다. **(CLI 버전은 변경 사항 없음)**

-   #### **⚡ 성능 최적화**
    -   **파일 액세스 구조 개선 (Caching):**
        -   파일을 분석할 때 열어둔 연결 통로(Handle)를 닫지 않고 메모리에 유지하여, 이후 재생이나 추출 요청 시 **파일을 다시 읽는 과정을 생략**합니다.
        -   덕분에 수천 개의 오디오가 담긴 대용량 `.bank` 파일에서도 버벅임 없이 **즉시 재생**이 가능해졌으며, 불필요한 디스크 읽기 작업을 줄였습니다.
    -   **일괄 추출 방식 개선:**
        -   파일을 하나 추출할 때마다 컨테이너를 열고 닫던 방식을 버리고, **같은 컨테이너에 속한 파일들을 한 번에 묶어서 처리**하도록 변경했습니다.
        -   컨테이너를 단 한 번만 연 상태에서 내부 사운드를 연속으로 뽑아내기 때문에 다량의 파일을 추출할 때 속도가 빨라졌습니다.

-   #### **🖱️ 사용자 경험 및 안정성**
    -   **자동 재생 및 엔진 안정성 강화:**
        -   `Auto-Play` 옵션을 켜고 빠르게 클릭할 때 소리가 정상적으로 나오지 않던 문제를 해결했습니다.
        -   **입력 지연(Debounce) 처리**로 불필요한 재생 요청을 거르고, 엔진 내부에는 리소스 정리 시간을 확보하여 연속 재생 시 발생하던 오류를 원천 차단했습니다.
...
이번 업데이트는 **리빌드 시스템의 편의성**과 **분석 로직의 안정성**을 대폭 강화하는 데 중점을 두었습니다. **(CLI 버전은 변경 사항 없음)**
...
## 💾 다운로드 <BR>
**⚠️ 저작권 및 라이선스 정책 준수를 위해, 이 리포지토리와 배포 파일에는 FMOD API 관련 소스 코드 및 바이너리 파일이 포함되어 있지 않습니다.** <BR> <BR>
...
- **아키텍처 선택 (v3.1.0 이상 GUI):** `CS_GUI` v3.1.0부터는 **AnyCPU**로 빌드되어 32비트와 64비트 환경을 모두 지원합니다.
  - 프로그램의 동작 모드는 사용자가 어떤 아키텍처의 FMOD 라이브러리(`.dll`)를 복사하는지에 따라 자동으로 결정됩니다.
  - **아래 표는 x64(64비트) 기준**이며, **32비트로 실행하려면 `api\...\lib\x86` 폴더의 `.dll` 파일을 사용**해야 합니다.
...
-   **[BekinTech](https://github.com/BekinTech)**
    -   19,597개의 오디오가 포함된 대용량 FSB 파일에서 발생하는 추출 속도 저하 문제를 구체적인 사례와 함께 제보해 주셨고, 원인 분석에 결정적인 샘플 파일까지 제공해 주셨습니다. 덕분에 WAV 헤더 표준화 로직을 반복 적용하는 과정에서 심각한 속도 저하가 발생하던 구간을 정확히 짚어낼 수 있었고, 해당 처리 흐름을 최적화하여 대용량 파일에서도 안정적인 추출 속도를 확보할 수 있었습니다.
